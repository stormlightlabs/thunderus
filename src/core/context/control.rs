...
pub enum ContextItemKind {
    ...
    /// schemas. System-owned; not editable through memory commands.
    Harness,
...
impl ContextItemKind {
    ...
    pub fn label(&self) -> &'static str {
        match self {
            ...
            ContextItemKind::UserMemory => "user_memory",
            ContextItemKind::ProjectMemory => "project_memory",
...
pub struct ContextItem {
    ...
    /// Short human-readable label (e.g. file path, memory title).
    pub label: String,
...
mod tests {
    ...
    fn item(id: &str, visibility: ContextVisibility) -> ContextItem {
        ContextItem {
            id: id.to_string(),
    ...
    fn fallback_limits() -> ModelContextLimits {
        ModelContextLimits {
            provider: "umans".to_string(),
            model: "umans-coder".to_string(),
            context_window: 200_000,
            max_completion_tokens: 32_768,
            recommended_completion_tokens: 8_192,
            source: ModelLimitSource::LiveMetadata,
            confidence: ModelLimitConfidence::Exact,
        }
    }

    /// 1. "hello" = 5 bytes -> ceil(5/3)=2 + 16 = 18
    /// 2. 0 bytes -> 0 + 16 = 16
    #[test]
    fn estimate_tokens_ascii() {
        assert_eq!(estimate_tokens(5), 18);
        assert_eq!(estimate_tokens(0), 16);
    }

    /// 1. "héllo" is 6 UTF-8 bytes (é is 2 bytes) -> ceil(6/3)=2 + 16 = 18
    /// 2. A 3-byte char (€) -> ceil(3/3)=1 + 16 = 17
    #[test]
    fn estimate_tokens_unicode_bytes_not_chars() {
        assert_eq!(estimate_tokens("héllo".len()), 18);
        assert_eq!(estimate_tokens("€".len()), 17);
    }

    /// 1. 100 bytes -> ceil(100/3)=34 + 16 = 50
    /// 2. 101 bytes -> ceil(101/3)=34 (101/3=33.66) + 16 = 50
    /// 3. 102 bytes -> ceil(102/3)=34 + 16 = 50
    /// 4. 103 bytes -> ceil(103/3)=35 + 16 = 51
    #[test]
    fn estimate_tokens_code_block_rounds_up() {
        assert_eq!(estimate_tokens(100), 50);
        assert_eq!(estimate_tokens(101), 50);
        assert_eq!(estimate_tokens(102), 50);
        assert_eq!(estimate_tokens(103), 51);
    }

    #[test]
    fn item_id_for_path_is_stable() {
        let path = Path::new("/repo/AGENTS.md");
        let a = item_id_for_path(&ContextItemKind::ProjectInstruction, path);
        let b = item_id_for_path(&ContextItemKind::ProjectInstruction, path);
        assert_eq!(a, b);
        assert!(a.starts_with("ctx_project_instruction_"));
    }

    #[test]
    fn item_id_for_path_differs_by_kind() {
        let path = Path::new("/repo/AGENTS.md");
        let instruction = item_id_for_path(&ContextItemKind::ProjectInstruction, path);
        let pinned = item_id_for_path(&ContextItemKind::PinnedFile, path);
        assert_ne!(instruction, pinned);
    }

    #[test]
    fn item_id_for_path_differs_by_path() {
        let root = item_id_for_path(&ContextItemKind::ProjectInstruction, Path::new("/repo/AGENTS.md"));
        let nested = item_id_for_path(&ContextItemKind::ProjectInstruction, Path::new("/repo/src/AGENTS.md"));
        assert_ne!(root, nested);
    }

    #[test]
    fn item_id_for_session_range_is_stable() {
        let a = item_id_for_session_range(&ContextItemKind::Transcript, "sess_1", 0, 10);
        let b = item_id_for_session_range(&ContextItemKind::Transcript, "sess_1", 0, 10);
        assert_eq!(a, b);
        assert!(a.starts_with("ctx_transcript_"));
    }

    #[test]
    fn item_id_for_session_range_differs_by_range() {
        let first = item_id_for_session_range(&ContextItemKind::Transcript, "sess_1", 0, 10);
        let second = item_id_for_session_range(&ContextItemKind::Transcript, "sess_1", 0, 11);
        assert_ne!(first, second);
    }

    #[test]
    fn item_id_for_session_range_differs_by_session() {
        let a = item_id_for_session_range(&ContextItemKind::Transcript, "sess_1", 0, 10);
        let b = item_id_for_session_range(&ContextItemKind::Transcript, "sess_2", 0, 10);
        assert_ne!(a, b);
    }

    #[test]
    fn resolve_uses_user_override_when_valid() {
        let live = LiveModelMetadata::new(200_000, 32_768, 8_192);
        let override_entry = ModelLimitOverride {
            context_window: 262_144,
            max_completion_tokens: 32_768,
            recommended_completion_tokens: 8_192,
        };
        let (limits, diagnostics) =
            ModelContextLimits::resolve("umans", "umans-coder", Some(override_entry), Some(&live));
        assert_eq!(limits.source, ModelLimitSource::UserOverride);
        assert_eq!(limits.context_window, 262_144);
        assert!(diagnostics.is_empty());
    }

    #[test]
    fn resolve_falls_back_to_live_when_override_invalid() {
        let live = LiveModelMetadata::new(200_000, 32_768, 8_192);
        let override_entry = ModelLimitOverride {
            context_window: 262_144,
            max_completion_tokens: 32_768,
            recommended_completion_tokens: 65_536, // exceeds max
        };
        let (limits, diagnostics) =
            ModelContextLimits::resolve("umans", "umans-coder", Some(override_entry), Some(&live));
        assert_eq!(limits.source, ModelLimitSource::LiveMetadata);
        assert_eq!(limits.context_window, 200_000);
        assert_eq!(diagnostics.len(), 1);
        assert_eq!(diagnostics[0].code, "invalid_model_override");
        assert_eq!(diagnostics[0].severity, DiagnosticSeverity::Error);
    }

    #[test]
    fn resolve_uses_live_metadata_when_no_override() {
        let live = LiveModelMetadata::new(200_000, 32_768, 8_192);
        let (limits, diagnostics) = ModelContextLimits::resolve("umans", "umans-coder", None, Some(&live));
        assert_eq!(limits.source, ModelLimitSource::LiveMetadata);
        assert_eq!(limits.context_window, 200_000);
        assert_eq!(limits.confidence, ModelLimitConfidence::Exact);
        assert!(diagnostics.is_empty());
    }

    #[test]
    fn resolve_uses_static_when_no_live_metadata() {
        let (limits, diagnostics) = ModelContextLimits::resolve("umans", "umans-coder", None, None);
        assert_eq!(limits.source, ModelLimitSource::Static);
        assert_eq!(limits.context_window, 200_000);
        assert!(diagnostics.is_empty());
    }

    #[test]
    fn resolve_uses_fallback_for_unknown_provider() {
        let (limits, diagnostics) = ModelContextLimits::resolve("unknown-provider", "some-model", None, None);
        assert_eq!(limits.source, ModelLimitSource::Fallback);
        assert_eq!(limits.context_window, FALLBACK_CONTEXT_WINDOW);
        assert_eq!(diagnostics.len(), 1);
        assert_eq!(diagnostics[0].code, "fallback_model_limits");
        assert_eq!(diagnostics[0].severity, DiagnosticSeverity::Warning);
    }

    #[test]
    fn resolve_falls_through_when_live_metadata_partial() {
        let live = LiveModelMetadata { context_window: None, ..Default::default() };
        let (limits, diagnostics) = ModelContextLimits::resolve("umans", "umans-coder", None, Some(&live));
        assert_eq!(limits.source, ModelLimitSource::Static);
        assert!(diagnostics.is_empty());
    }
    ...
    #[test]
    fn resolve_rejects_zero_override() {
        let override_entry = ModelLimitOverride {
            context_window: 0,
            max_completion_tokens: 32_768,
            recommended_completion_tokens: 8_192,
        };
        let (limits, diagnostics) = ModelContextLimits::resolve("umans", "umans-coder", Some(override_entry), None);
        assert_eq!(limits.source, ModelLimitSource::Static);
        assert_eq!(diagnostics.len(), 1);
        assert!(diagnostics[0].message.contains("context_window"));
    }

    #[test]
    fn resolve_rejects_recommended_exceeding_max() {
        let override_entry = ModelLimitOverride {
            context_window: 262_144,
            max_completion_tokens: 8_192,
            recommended_completion_tokens: 32_768,
        };
        let (limits, diagnostics) = ModelContextLimits::resolve("umans", "umans-coder", Some(override_entry), None);
        assert_eq!(limits.source, ModelLimitSource::Static);
        assert_eq!(diagnostics.len(), 1);
        assert!(diagnostics[0].message.contains("recommended_completion_tokens"));
    }

    #[test]
    fn resolve_rejects_max_not_less_than_context_window() {
        let override_entry = ModelLimitOverride {
        ...
        assert_eq!(limits.source, ModelLimitSource::Static);
        assert_eq!(diagnostics.len(), 1);
        assert!(diagnostics[0].message.contains("max_completion_tokens"));
    }

    /// 200_000 - max(8_192, 32_768) - 1_024 = 200_000 - 32_768 - 1_024
    #[test]
    fn available_input_budget_subtracts_completion_and_overhead() {
        ...
        assert_eq!(limits.available_input_budget(), 200_000 - 32_768 - 1_024);
    }

    #[test]
    fn target_and_auto_compaction_derived_from_available_input() {
        let limits = ModelContextLimits {
            provider: "umans".to_string(),
            model: "umans-coder".to_string(),
            context_window: 200_000,
        ...
        let available = limits.available_input_budget();
        assert_eq!(limits.target_budget(), (available as f64 * 0.80).floor() as u64);
        assert_eq!(
            limits.auto_compaction_threshold(),
            (available as f64 * 0.92).floor() as u64
        );
    }

    #[test]
    fn target_is_not_raw_context_window() {
        let limits = ModelContextLimits {
            provider: "umans".to_string(),
            model: "umans-coder".to_string(),
            context_window: 200_000,
        ...
        assert_ne!(limits.target_budget(), (200_000.0 * 0.80) as u64);
        assert!(limits.target_budget() < 200_000);
    }

    #[test]
    fn budget_tracks_rendered_items_only() {
        let limits = ModelContextLimits {
            provider: "umans".to_string(),
            model: "umans-coder".to_string(),
            context_window: 200_000,
        ...
        let items = vec![
            ...
            ContextItem {
                id: "ctx_pinned_1".to_string(),
                kind: ContextItemKind::PinnedFile,
                label: "src/lib.rs".to_string(),
                source_path: Some(PathBuf::from("/repo/src/lib.rs")),
                scope: "src".to_string(),
            ...
            ContextItem {
                id: "ctx_archived_1".to_string(),
                kind: ContextItemKind::ToolArchive,
                label: "old tool output".to_string(),
                source_path: None,
                scope: ".".to_string(),
                content_hash: None,
                byte_count: 300,
        ...
        assert_eq!(budget.used, 50 + 80);
    }

    /// available = 10_000 - 1_000 - 1_024 = 7_976
    /// target = floor(7_976 * 0.80) = 6_380
    /// auto = floor(7_976 * 0.92) = 7_337
    #[test]
    fn budget_flags_target_and_auto_compaction_pressure() {
        let limits = ModelContextLimits {
            provider: "umans".to_string(),
            model: "umans-coder".to_string(),
            context_window: 10_000,
            max_completion_tokens: 1_000,
            recommended_completion_tokens: 500,
            source: ModelLimitSource::LiveMetadata,
            confidence: ModelLimitConfidence::Exact,
        };
        let items = vec![ContextItem {
            id: "ctx_big_1".to_string(),
            kind: ContextItemKind::PinnedFile,
            label: "big".to_string(),
            source_path: None,
            scope: ".".to_string(),
        ...
        let budget = ContextBudget::from_limits(limits, &items);
        assert!(budget.exceeds_target());
        assert!(!budget.exceeds_auto_compaction());
    }

    #[test]
    fn ledger_counts_group_by_visibility() {
        let limits = ModelContextLimits {
            provider: "umans".to_string(),
            model: "umans-coder".to_string(),
            context_window: 200_000,
        ...
        let items = vec![
            item("a", ContextVisibility::Visible),
            item("b", ContextVisibility::Visible),
            item("c", ContextVisibility::Pinned),
            item("d", ContextVisibility::Archived),
            item("e", ContextVisibility::Candidate),
        ];
        let budget = ContextBudget::from_limits(limits, &items);
        let ledger = ContextLedger { items, budget, diagnostics: Vec::new() };
        let counts = ledger.counts();
    ...
    #[test]
    fn ledger_find_by_id() {
        let limits = fallback_limits();
        let items = vec![item("ctx_x_1", ContextVisibility::Visible)];
        let budget = ContextBudget::from_limits(limits, &items);
        let ledger = ContextLedger { items, budget, diagnostics: Vec::new() };
        assert!(ledger.find("ctx_x_1").is_some());
        assert!(ledger.find("missing").is_none());
    }

    #[test]
    fn ledger_summary_includes_visible_pinned_and_tokens() {
        let limits = fallback_limits();
        let items = vec![
            item("a", ContextVisibility::Visible),
            item("b", ContextVisibility::Visible),
        ...
        let budget = ContextBudget::from_limits(limits, &items);
        let ledger = ContextLedger { items, budget, diagnostics: Vec::new() };
        let summary = render_ledger_summary(&ledger);
        assert!(summary.starts_with("context  "));
        assert!(summary.contains("2 visible"));
        assert!(summary.contains("1 pinned"));
        assert!(summary.contains("1 archived"));
        assert!(summary.contains("est. tokens"));
    }

    #[test]
    fn model_dashboard_excludes_full_content() {
        let limits = fallback_limits();
        let items = vec![ContextItem {
            id: "ctx_secret_1".to_string(),
            kind: ContextItemKind::UserMemory,
        ...
        let budget = ContextBudget::from_limits(limits, &items);
        let ledger = ContextLedger { items, budget, diagnostics: Vec::new() };
        let dashboard = render_model_dashboard(&ledger);
        assert!(dashboard.contains("<context_dashboard>"));
        assert!(dashboard.contains("ctx_secret_1"));
        assert!(dashboard.contains("user_memory"));
        assert!(dashboard.contains("visible"));
        assert!(dashboard.contains("<budget>"));
        assert!(dashboard.contains("limit_source"));
        assert!(!dashboard.contains("byte_count"));
        assert!(!dashboard.contains("content_hash"));
    }

    #[test]
    fn model_dashboard_includes_diagnostics() {
        let limits = fallback_limits();
        let items = vec![item("a", ContextVisibility::Visible)];
        let budget = ContextBudget::from_limits(limits, &items);
        let diagnostics = vec![ContextDiagnostic::fallback_model_limits("unknown", "model")];
        let ledger = ContextLedger { items, budget, diagnostics };
        let dashboard = render_model_dashboard(&ledger);
        assert!(dashboard.contains("<diagnostics>"));
        assert!(dashboard.contains("fallback_model_limits"));
    }

    #[test]
    fn context_item_summary_is_compact() {
        let item = ContextItem {
            id: "ctx_pinned_abc".to_string(),
            kind: ContextItemKind::PinnedFile,
            label: "src/lib.rs".to_string(),
            source_path: Some(PathBuf::from("/repo/src/lib.rs")),
            scope: "src".to_string(),
        ...
        assert!(summary.contains("ctx_pinned_abc"));
        assert!(summary.contains("pinned_file"));
