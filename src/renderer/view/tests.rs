...
fn build_view_streaming_assistant_is_all_live() {
    ...
    assert!(
        !view
            ...
            .stable_rows
            .iter()
            .any(|row| row.text().contains("Assistant") || row.text().contains("line one")),
    ...
    assert!(
        ...
        "live_rows should contain the assistant header"
    );
}
...
fn build_view_tiny_height_clips_live_tail() {
    ...
    app.transcript.push(Entry::Assistant { text, streaming: true });
