...
///
/// The source text is deliberately held only in memory. Callers persist the
...
mod tests {
    ...
    fn defaults_are_manual_and_auto_review() {
        let config: ContextConfig = toml::from_str("[compaction]\n").expect("parse config");
        ...
        for mode in ["off", "manual", "auto"] {
            for review in ["always", "auto", "never"] {
                let config: ContextConfig =
                    toml::from_str(&format!("[compaction]\nmode = \"{mode}\"\nreview = \"{review}\"\n"))
                        .expect("parse config");
                assert_eq!(
