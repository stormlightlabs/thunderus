//! WASM tool wrapper - integrates WASM plugins with the Tool system.
//!
//! WASM plugins are wrapped as Tool implementations so they can be registered
...
/// A WASM plugin wrapped as a Tool for integration with the tool registry.
#[derive(Debug, Clone)]
pub struct WasmTool {
    /// The skill metadata
    meta: Arc<SkillMeta>,
    ...
    /// Permissions for the WASM plugin
...
impl WasmTool {
    /// Create a new WasmTool from a Skill.
    ///
    /// Returns None if the skill doesn't have a WASM driver.
    pub fn new(skill: Skill) -> Option<Self> {
        if skill.meta.driver != thunderus_skills::SkillDriver::Wasm {
            return None;
        }

        let entry = if skill.meta.entry.is_empty() {
            format!("{}.wasm", skill.meta.name)
        } else {
            skill.meta.entry.clone()
        };

        let wasm_path = skill.meta.path.join(&entry);
    ...
    fn execute_wasm_plugin(&self, arguments: &Value) -> Result<ToolResult> {
        #[cfg(feature = "wasm")]
        {
            let mut engine = thunderus_skills::WasmEngine::new();
            ...
            engine
                .load(self.meta.name.clone(), &self.wasm_path, context, functions)
        ...
        #[cfg(not(feature = "wasm"))]
        {
            Ok(ToolResult {
                tool_call_id: String::new(),
                content: "WASM support is not enabled. Build with --features wasm to enable WASM plugins.".to_string(),
...
impl Tool for WasmTool {
    ...
    fn classification(&self) -> Option<Classification> {
        ...
        let reasoning = match risk {
            ToolRisk::Safe => format!("WASM plugin '{}' is marked as safe", self.meta.name),
            ToolRisk::Risky => format!(
                "WASM plugin '{}' may perform network or file operations - risky",
                self.meta.name
            ),
            ToolRisk::Blocked => format!("WASM plugin '{}' is blocked", self.meta.name),
        };
    ...
    fn execute(&self, tool_call_id: String, arguments: &Value) -> Result<ToolResult> {
        let mut result = self.execute_wasm_plugin(arguments)?;
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::new(
            self.tool_name(),
            format!("WASM Plugin: {}", self.meta.description),
            self.parameters(),
        )
    }
}
...
impl WasmTool {
    fn tool_parameters(&self) -> ToolParameter {
        ...
        ToolParameter::new_object(vec![(
            "input".to_string(),
            ToolParameter::new_string("Input data for the WASM plugin"),
        )])
    }
...
fn tool_parameter_from_schema(schema: &Value) -> ToolParameter {
    ...
    match schema_type {
        ...
        _ => ToolParameter::new_string("Input data for the WASM plugin"),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_wasm_tool_requires_wasm_driver() {
...
"#,
        ...
        assert!(WasmTool::new(skill).is_none());
    }
}
