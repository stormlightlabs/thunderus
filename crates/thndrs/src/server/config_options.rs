...
impl core::fmt::Display for ConfigOptionError {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::UnknownOption { id } => write!(formatter, "unknown config option `{id}`"),
            Self::InvalidValue { id, value, reason } => {
                write!(formatter, "invalid value `{value}` for config option `{id}`: {reason}")
            }
        }
    }
}
...
/// Return initial ACP config option IDs in protocol response order.
pub fn initial_config_option_ids() -> &'static [&'static str; 3] {
    &[
...
/// Return initial server config option specs.
pub fn initial_config_options() -> &'static [ConfigOptionSpec] {
    const OPTIONS: [ConfigOptionSpec; 3] = [
...
pub fn acp_config_options(model: &str, effort: ReasoningEffort, summary: ReasoningSummary) -> Vec<SessionConfigOption> {
    vec![
...
fn reasoning_effort_config_option(model: &str, effort: ReasoningEffort) -> SessionConfigOption {
    let options = crate::providers::reasoning_options(model)
        .into_iter()
