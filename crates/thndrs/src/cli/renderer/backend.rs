...
//! a [`Vec<u8>`] buffer. The backend owns cursor visibility and shape,
//! viewport clearing, cursor movement, and styled row output.
//!
...
impl<W: Write> TerminalBackend<W> {
    ...
    /// scrollback.
    ///
    /// Scrolls the full terminal by one physical row per committed row, then
    /// paints the new row directly above the live region. Full-screen scrolling
    /// is important: terminals commonly discard lines scrolled out of a
    /// partial DECSTBM region instead of preserving them in scrollback.
    ///
    ...
    pub fn insert_history_lines(&mut self, rows: &[Row], viewport_top: u16) -> io::Result<()> {
        ...
        if viewport_top == 0 || self.height == 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "history insertion requires a row above the live region",
            ));
        }

        queue!(self.writer, ResetScrollRegion)?;
        let history_row = viewport_top.saturating_sub(1);
        for row in rows {
            queue!(self.writer, MoveTo(0, self.height.saturating_sub(1)))?;
            queue!(self.writer, crossterm::style::Print("\r\n"))?;
            queue!(self.writer, MoveTo(0, history_row))?;
    ...
    pub fn render_frame(&mut self, frame: &Frame, top_row: u16) -> io::Result<()> {
        ...
        if frame.cursor_visible {
            if let Some(cursor) = frame.cursor {
                ...
                queue!(self.writer, Show, SetCursorStyle::BlinkingBar)?;
            } else {
    ...
    pub fn render_frame_diff(&mut self, frame: &Frame, prev: Option<&Frame>, top_row: u16) -> io::Result<()> {
        ...
        if frame.cursor_visible {
            ...
            } else if prev.is_none_or(|p| p.cursor.is_some() && p.cursor_visible) {
                queue!(self.writer, Hide)?;
            }
        } else {
...
mod tests {
    ...
    fn insert_history_lines_scrolls_before_first_row() {
        ...
        assert!(
            ...
            "partial scroll regions do not reliably feed native scrollback"
        );
        assert!(
    ...
    fn history_insert_then_live_diff_renders_viewport() {
        ...
        assert!(
            ...
            "history insert should not use a partial scroll region"
        );
        assert!(
            total.contains("\x1b[10;1H\r\n"),
            "history insert should scroll the full terminal"
        );
        assert!(
